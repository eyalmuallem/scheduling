...
import { Lock, Eye, EyeOff, Send, Users, AlertTriangle, Plus, Trash2, Info } from 'lucide-react';
...
import { 
  ...
  setConstraintSubmissionLocked,
} from '@/app/actions/schedule';
import { DAYS_OF_WEEK, TASKS, TASK_COLORS, TaskType, getWeekStart, getWeekDates, formatDateShort } from '@/lib/types';
import { cn } from '@/lib/utils';
import {
  buildWarehouseAssignmentDetails,
  isWarehouseAssignmentDetails,
  parseWarehouseAssignment,
} from '@/lib/warehouse-assignment';
import type { WarehouseAssignmentKind } from '@/lib/warehouse-assignment';
...
const buildAssignmentDetails = (eventTitle: string, details: string) => {
  const cleanTitle = eventTitle.trim();
  const cleanDetails = details.trim();

  if (!cleanTitle) return cleanDetails || undefined;
  return `${EVENT_TITLE_PREFIX} ${cleanTitle}${EVENT_DETAILS_SEPARATOR}${cleanDetails}`;
};
...
interface ConstraintSubmissionLocks {
  soundman: boolean;
  warehouse: boolean;
}

type ConstraintGroup = keyof ConstraintSubmissionLocks;
...
export default function CommanderPage() {
  ...
  const [updatingConstraintLock, setUpdatingConstraintLock] = useState<ConstraintGroup | null>(null);
  ...
  const { data: constraintLocks = { soundman: false, warehouse: false }, mutate: mutateConstraintLocks } = useSWR<ConstraintSubmissionLocks>(
  ...
  const handleToggleConstraintLock = async (group: ConstraintGroup) => {
    setUpdatingConstraintLock(group);
    try {
      await setConstraintSubmissionLocked(weekStart, group, !constraintLocks[group]);
      await mutateConstraintLocks();
    } catch (error) {
      console.error('Error updating constraint submission lock:', error);
      alert('שגיאה בעדכון נעילת הגשת האילוצים');
    } finally {
      setUpdatingConstraintLock(null);
    }
  };
  ...
  const handleSetWarehouseAssignment = async (
    soldierId: number,
    dayOfWeek: number,
    ...
    if (kind === 'משמרת' && (!startTime || !endTime)) return;
    if (kind === 'שונות' && !note.trim()) return;

    await handleSetAssignment(
      soldierId,
      dayOfWeek,
      kind === 'שונות' ? 'שונות' : 'נוכחות',
      buildWarehouseAssignmentDetails(kind, { startTime, endTime, note })
    );
  };
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                כל נעילה חלה רק על הקבוצה שלה בשבוע הנבחר. פרסום השבוע נשאר משותף ונועל את שתי הקבוצות.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {([
                  { group: 'soundman' as const, label: 'סאונדמנים' },
                  { group: 'warehouse' as const, label: 'מחסנאים' },
                ]).map(({ group, label }) => {
                  const isLocked = constraintLocks[group];
                  const isUpdating = updatingConstraintLock === group;

                  return (
                    <div key={group} className="flex items-center justify-between gap-3 rounded-lg border p-3">
                      <div>
                        <div className="font-medium">{label}</div>
                        <div className={cn('text-sm', isLocked ? 'text-red-700' : 'text-emerald-700')}>
                          {isLocked ? 'הגשת האילוצים חסומה' : 'הגשת האילוצים פתוחה'}
                        </div>
                      </div>
                      <Button
                        type="button"
                        ...
                        size="sm"
                        disabled={isUpdating}
                        onClick={() => handleToggleConstraintLock(group)}
                      >
                        <Lock className="h-4 w-4 ml-2" />
                        {isUpdating ? 'מעדכן...' : isLocked ? 'פתח הגשה' : 'חסום הגשה'}
                      </Button>
                    </div>
                  );
                })}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                ברירת המחדל היא משמרת שעתית. לפי הצורך אפשר לבחור גם נוכחות או שונות ולהוסיף הערה קצרה, בלי שינוי במבנה הדאטהבייס.
              </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            return (
                              ...
                              >
                                <WarehouseWorkerCell
                                  ...
                                  constraints={cellConstraints}
                                  showConstraints={showConstraints}
                                  onSave={(kind, startTime, endTime, note) =>
                                    handleSetWarehouseAssignment(worker.id, dayIndex, kind, startTime, endTime, note)
                                  }
...
interface WarehouseWorkerCellProps {
  ...
  onSave: (kind: WarehouseAssignmentKind, startTime: string, endTime: string, note: string) => void;
  onRemove: () => void;
}
...
function WarehouseWorkerCell({
  ...
  const parsedAssignment = parseWarehouseAssignment(assignment?.details);
  const [kind, setKind] = useState<WarehouseAssignmentKind>(parsedAssignment.kind);
  const [startTime, setStartTime] = useState(parsedAssignment.startTime);
  const [endTime, setEndTime] = useState(parsedAssignment.endTime);
  const [note, setNote] = useState(parsedAssignment.note);

  useEffect(() => {
    const latestAssignment = parseWarehouseAssignment(assignment?.details);
    setKind(latestAssignment.kind);
    setStartTime(latestAssignment.startTime);
    setEndTime(latestAssignment.endTime);
    setNote(latestAssignment.note);
  }, [assignment?.details]);
  ...
  return (
    <div className="space-y-2 text-right">
      ...
      <div className="space-y-1">
        <Label className="text-xs">סוג שיבוץ</Label>
        <Select value={kind} onValueChange={(value) => setKind(value as WarehouseAssignmentKind)}>
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="משמרת">משמרת</SelectItem>
            <SelectItem value="נוכחות">נוכחות</SelectItem>
            <SelectItem value="שונות">שונות</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {kind === 'שונות' && (
        <div className="space-y-1">
          <Label className="text-xs">הערה קצרה</Label>
          <Input
            value={note}
            onChange={(event) => setNote(event.target.value)}
            placeholder="מילה או שתיים..."
            maxLength={50}
          />
        </div>
      )}
      ...
      {assignment && (
        <div className={cn(
          "text-xs rounded px-2 py-1 text-center border",
          parsedAssignment.kind === 'שונות'
            ? "text-gray-700 bg-gray-50 border-gray-200"
            : "text-emerald-700 bg-emerald-50 border-emerald-200"
        )}>
          <div className="font-medium">
            נשמר: {parsedAssignment.kind === 'משמרת'
              ? `${parsedAssignment.startTime} - ${parsedAssignment.endTime}`
              : parsedAssignment.kind}
          </div>
          {parsedAssignment.kind === 'שונות' && parsedAssignment.note && (
            <div className="mt-0.5 break-words font-normal">{parsedAssignment.note}</div>
          )}
        </div>
      )}
    </div>
...
function AssignmentCell({ 
  ...
  const currentDetails = parseAssignmentDetails(assignment?.details);
  const [selectedTask, setSelectedTask] = useState<TaskType | ''>('');
  const [eventTitle, setEventTitle] = useState(currentDetails.eventTitle);
  const [details, setDetails] = useState(currentDetails.details);
  const [isOpen, setIsOpen] = useState(false);

  const serializedDetails = buildAssignmentDetails(eventTitle, details);
  const hasDetailsChanged = (serializedDetails || '') !== (assignment?.details || '');
  ...
  const handleSave = () => {
    ...
    onAssign(taskToSave, serializedDetails);
    setSelectedTask('');
    setIsOpen(false);
  };
  ...
  const handleOpenChange = (open: boolean) => {
    setIsOpen(open);
    if (open) {
      const nextDetails = parseAssignmentDetails(assignment?.details);
      setSelectedTask('');
      setEventTitle(nextDetails.eventTitle);
      setDetails(nextDetails.details);
    }
  };
    ...
    <Popover open={isOpen} onOpenChange={handleOpenChange}>
        ...
        <button className="w-full min-h-[60px] p-1 hover:bg-accent/50 rounded transition-colors">
          ...
          {assignment ? (
            ...
            >
              ...
              {currentDetails.eventTitle && (
                <div className="mt-1 truncate rounded bg-white/50 px-1.5 py-0.5 text-[11px] font-semibold leading-snug underline-offset-2">
                  {currentDetails.eventTitle}
                </div>
              )}
              {!currentDetails.eventTitle && currentDetails.details && assignment.task === 'חזרות' && (
                <div className="mt-1 whitespace-pre-wrap break-words text-[11px] font-normal leading-snug opacity-80">
                  {currentDetails.details}
                </div>
              )}
              {currentDetails.details && assignment.task !== 'חזרות' && !currentDetails.eventTitle && (
                <div className="text-[11px] opacity-75 mt-1">יש פירוט נוסף</div>
              )}
            </div>
          ) : (
      ...
      <PopoverContent className="w-72" align="center">
        <div className="space-y-4">
          ...
          {(currentDetails.eventTitle || currentDetails.details) && (
            <div className="rounded-md border bg-muted/40 p-3 text-sm">
              ...
              {currentDetails.eventTitle && (
                <div className="mb-2 font-semibold text-foreground">{currentDetails.eventTitle}</div>
              )}
              {currentDetails.details && (
                <div className="max-h-40 overflow-y-auto whitespace-pre-wrap break-words text-muted-foreground leading-relaxed">
                  {currentDetails.details}
                </div>
              )}
            </div>
          ...
          <div className="space-y-2">
            <Label>שם אירוע (אופציונלי)</Label>
            <Input
              placeholder="לדוגמה: הופעה"
              value={eventTitle}
          ...
          <div className="space-y-2">
            ...
            <Textarea 
              placeholder="הוסף פרטים ארוכים יותר..."
              value={details}
